...
public class OnePass {
	...
	private ArrayList<String> titlelist
		                    = new ArrayList<String>();//找出所有的标题，然后从原有的list_wordTFDF获取包含title的拿下来存储到新的featurelist下；
...
//		}
	 ...
	 */
	/***************************************************************************/
	public void getFeatureWordbyTitle()
	{
		...
		{
			//处理标点，去除停用词
			String newstr=proProcessTool.processTitle(articles.get(j).filename);
			//System.out.println(newstr);
			String [] tem=newstr.split(" ");
			String result="";
			for(String word : tem)
			{
				if(word!=null && !HotSpot.getStopWord().contains(word))
				{
					result+=word+" ";					
				}
			}
		...
		FeatureWordSize = featureWordMap.size();
		//printF();
		System.out.println(FeatureWordSize);
